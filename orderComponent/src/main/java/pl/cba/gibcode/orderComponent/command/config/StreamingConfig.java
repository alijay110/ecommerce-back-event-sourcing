...
public class StreamingConfig extends KafkaConfig {
	...
	public KStream<String, OrderComponentEvent> orderComponentStream(
		...
		orderStream.to("order", CustomJsonSerde.produce(Order.class));
		...
		brandStream.to("brand", CustomJsonSerde.produce(Brand.class));

		var cardStream = transformedStream.mapValues((s, processingWrapper) -> processingWrapper.getCard())
				.filter(((s, card) -> nonNull(card)))
				.peek((key, value) -> logger.info("card: {}: {}", key, value));
		cardStream.to("card", CustomJsonSerde.produce(Card.class));
		queryKafkaConfig.prepareQueries(orderStream, brandStream, cardStream);
	...
	private class Transformer implements
		...
		@Override public ProcessingWrapper transform(String key, OrderComponentEvent event) {
			...
			if(nonNull(existingOrderComponentEvent) && event.getHeader().getAction()
					.equals(existingOrderComponentEvent.getHeader().getAction())
					&& event.getHeader().getCreationTimestamp()
					.isBefore(existingOrderComponentEvent.getHeader().getCreationTimestamp())) {
				processingWrapper
						.setResponse(ModelBuilderUtils.buildFailureOrder(event, "Event timestamp incorrect.",
								UNKNOWN));
			...
			if(nonNull(orderStore.get(key)) && !StateMachine
				...
				processingWrapper.setResponse(ModelBuilderUtils.buildFailureOrder(event,
						"Event transition incorrect:" + orderStore.get(key).getEntityState() + " -> " + event
